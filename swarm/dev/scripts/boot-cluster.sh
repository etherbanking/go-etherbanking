...
#
# The cluster contains a bootnode, a gebc node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gebc and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gebc node with --datadir pointing at <base-dir>/gebc
# and a single, unlocked account with password "gebc"
start_geth_node() {
  local dir="${base_dir}/gebc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gebc account address"
  fi
  ...
  start_node "gebc" "${GETH_IP}" "$(which gebc)" ${args[@]}
}
...
create_account() {
  ...
  gebc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
