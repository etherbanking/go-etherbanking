...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gebc.LightScryptN, Gebc.LightScryptP);
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gebc-android-")
	if err != nil {
	...
	// Generate the mobile bindings for Gebc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/etherbanking/go-etherbanking/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gebc.aar"), "gebc.aar", os.ModePerm)
